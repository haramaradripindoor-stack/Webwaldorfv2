    ...
    for (const dir of dirs) {
        ...
        if (fs.existsSync(portFile)) {
            try {
                ...
                if (page) {
                    console.log(`👁️ CapCut encontrado. Aprobando el plan de video...`);
                    await page.bringToFront();
                    
                    // Buscar el botón por texto "Approve:" o "Aprobar"
                    const btn = await page.evaluateHandle(() => {
                        const btns = Array.from(document.querySelectorAll('button, div[role="button"]'));
                        return btns.find(b => b.innerText && (b.innerText.includes('Approve') || b.innerText.includes('Aprobar plan') || b.innerText.includes('Approve: Start generation')));
                    });
                    
                    if (btn) {
                        await btn.click();
                        console.log("✅ Clic en 'Approve' ejecutado.");
                    } else {
                        console.log("❌ No encontré el botón de Approve. Tal vez cambió la UI.");
                    }
